...
```bash
EventNotifier$ sam local invoke helloFromLambdaFunction --no-event
```
...
```bash
EventNotifier$ sam logs -n helloFromLambdaFunction --stack-name sam-app --tail
```
...
```bash
EventNotifier$ dotnet test test/EventNotifier.Tests/EventNotifier.Tests.csproj 
```
...
```bash
sam delete --stack-name EventNotifier
```
